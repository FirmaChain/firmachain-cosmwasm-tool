...
import { useDecreaseAllowance } from "hooks/cw20/transaction";
import { DECREASE_ALLOWANCE_LOADING, DECREASE_ALLOWANCE_SUCCESS } from "constants/message";
...
import { getDecreaseAllowanceFee } from "../tx/decreaseAllowance";
...
const DecreaseAllowance = ({ firmaSDK, wallet, address, contractAddress }: IProps) => {
  ...
  const { mutate, isLoading } = useDecreaseAllowance(firmaSDK, wallet, contractAddress, spender, adjustValueByDecimal(amount, Cw20Decimal), expires, {
    onMutate: () => {
      LoadingProgress({ enable: true, message: DECREASE_ALLOWANCE_LOADING });
    },
    onSuccess: (data: any) => {
      Toast({ message: DECREASE_ALLOWANCE_SUCCESS, variant: "success" });
    },
    onError: (error: any) => {
  ...
  useEffect(() => {
    LoadingProgress({ enable: isLoading, message: isLoading ? DECREASE_ALLOWANCE_LOADING : undefined });
  }, [isLoading]);
  ...
  const onClickDecreaseAllowance = async () => {
    try {
      ...
      const fee = await getDecreaseAllowanceFee({ firmaSDK, wallet, contractAddress, spender, amount: formatAmount, expires });

      openModal(
        <Fragment>
          <Stack gap={"10px"}>
            <LabelDisplay label={"Type (Trx.)"} value={"DecreaseAllowance"} />
          ...
          <Stack flexDirection={"column"} gap={"0px"}>
            ...
            <Stack flexDirection={"row"} gap={"10px"}>
              ...
              <Stack flexDirection={"row"} gap={"10px"} sx={{ width: "250px" }}>
                <SmallButton title={"Allowance"} active={active} onClick={onClickDecreaseAllowance} />
              </Stack>
            </Stack>
          </Stack>
        }
